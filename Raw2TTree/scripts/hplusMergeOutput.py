...
Merges ROOT files into one (or more) files. It also reads TopPt.root and adds a "top-pt-correction-weigh" histogram in raw2TTree.root files. 
...
def delete(fileName, regexp, opts):
    ...
    TFile *f = TFile::Open("root://cmseos.fnal.gov//store/user/aattikis/CRAB3_TransferData/WWTo4Q_13TeV-powheg/crab_WWTo4Q/160920_123933/0000/raw2TTree_1.root");
...
def GetTaskRootFiles(taskName, basename, opts):
    ...
    if len(rootFilesList) < 1:
        ...
        rootFilesList = [pathName + f for f in ret if "raw2TTree" in f]    
