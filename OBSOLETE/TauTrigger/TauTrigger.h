...
class TauTrigger : public TreeAnalyserMC{
 public:
  // Constructors/Destructors
 TauTrigger(const string SamplePath,
	    const string SampleName,
	    const string text_, 
	    const int maxEvents_ = -1, 
	    TTree* tree=0) : 
  
  TreeAnalyserMC("TauTrigger", SamplePath, SampleName, text_, maxEvents_, tree) 
    { 
  ...
  void GetL1TkTauSigConePixTracks(const int ldgTk_index, 
				  const double deltaR_min,
				  const double deltaR_max,
				  const string selectionType,
				  vector<int> &sigTks_index);
