#ifndef PixelTauTrigger_cxx
#define PixelTauTrigger_cxx

#include "PixelTauTrigger.h"
#include "../utilities/constants.h"
#include "TFitResult.h"
#include "TF1.h"

//****************************************************************************
void PixelTauTrigger::InitObjects(void)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::InitVars_()
//****************************************************************************
{
  ...
  // L1TkTau 
  tk_CollectionType        = "TTPixelTracks"; // "TTTracks"; // "TTPixelTracks"; // "TTPixelTracks-Refit";
  ...
  sigCone_ShrinkConstant   = +1.50;      // TP: 0.00
  sigCone_maxDeltaR        = +0.15;      // TP: 0.15
  ...
  isoCone_minDeltaR        = sigCone_maxDeltaR;  // TP: 0.4cm
  isoCone_maxDeltaR        = +0.30;              // TP: 0.4cm
...
//****************************************************************************
void PixelTauTrigger::PrintSettings(void)
//****************************************************************************
{
...
//****************************************************************************
{
  ...
  for (int jentry=0; jentry < nEntries; jentry++){
      ...
      if ( tk_CollectionType.compare("TTPixelTracks") == 0 )
	{
      ...
      else if ( tk_CollectionType.compare("TTPixelTracks-Refit") == 0 )
	{

	  GetL1TkTauMatchPixTrackRefit(matchTk_index, iCalo, sigCone_tkCollectionWP, matchTk_minPt_, matchTk_caloDeltaR_, matchTk_deltaR_tmp, f);
	  GetL1TkTauAlgoConeSizes(iCalo, tauAlgorithmMode, sigCone_maxDeltaR, sigCone_minDeltaR_tmp, sigCone_maxDeltaR_tmp, isoCone_minDeltaR_tmp);

	  GetL1TkTauSigConePixTracksRefit(matchTk_index, sigCone_minDeltaR_tmp, sigCone_maxDeltaR_tmp, sigCone_tkCollectionWP, sigTks_index, f); 
	  GetL1TkTauIsoConePixTracksRefit(matchTk_index, isoCone_minDeltaR_tmp, isoCone_maxDeltaR    , isoCone_tkCollectionWP, isoTks_index, f);
	   
	  GetL1TkTauPixIsolationRefit(matchTk_index, isoTks_index, relIso, vtxIso, f);
	}      
	...
	{
	  cout << "E R R O R ! PixelTauTrigger::Loop(...) - Invalid Track Collection Type \"" << tk_CollectionType << "\". EXIT" << endl;
	  exit(1);
	}
...
//****************************************************************************
void PixelTauTrigger::BookHistos_(void)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::FinaliseEffHisto_(TH1D *histo, 
...
//****************************************************************************
void PixelTauTrigger::FinaliseEffHisto_(TH2D *histo, 
				   const int nEvtsTotal)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::ApplyDiTauZMatching(const string tkCollectionType, 
...
//****************************************************************************
{
  ...
  for( int i=1; i < (int) L1TkTaus.size(); i++){
    ...
    if ( tkCollectionType.compare("TTPixelTracks") == 0 ) {
    ...
    else if ( tkCollectionType.compare("TTPixelTracks-Refit") == 0 ) {

      if(DEBUG) cout << "I N F O! PixelTauTrigger::ApplyDiTauZMatching(...) - TTPixel Tracks not refitted. Needs fix" << endl; //fixme alex
      deltaPOCAz = abs( L1PixTks_POCAz->at(matchTk_index) - L1PixTks_POCAz->at(matchTk_index0) );
    }
      ...
      cout << "E R R O R ! PixelTauTrigger::ApplyDiTauZMatching(...) - Unknown sample \"" << mcSample << "\". EXIT" << endl;
      exit(1);
    }
...
//****************************************************************************
void PixelTauTrigger::FillSingleTau_(const vector<L1TkTauParticle> L1TkTaus, 
				TH1D *hRate,
				TH1D *hEfficiency)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::FillDiTau_(const vector<L1TkTauParticle> L1TkTaus, 
			    TH1D *hRate,
			    TH1D *hEfficiency)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::FillDiTau_(const vector<L1TkTauParticle> L1TkTaus1,
...
//****************************************************************************
{
  ...
  if (ldgEt1 > ldgEt2)      FillRate_(hRate, ldgEt1, ldgEt2); 
    ...
    if (L1TkTaus1_.at(0).caloTau_Index_ == L1TkTaus2.at(0).caloTau_Index_ ) {
      cout << "E R R O R ! PixelTauTrigger::FillDiTau_(...) - Leading and Sub-Leading Trigger objects have the same eT (" << ldgEt1 << " , " << ldgEt2 << ")! "
  ...
  if (ldgEt_mcMatched1 > ldgEt_mcMatched2)      histoTools_.FillAllBinsUpToValue_2D(hEfficiency, ldgEt_mcMatched1, ldgEt_mcMatched2);
    ...
    if (L1TkTaus_mcMatched1.at(0).caloTau_Index_ == L1TkTaus_mcMatched2.at(0).caloTau_Index_ ) {
      cout << "E R R O R ! PixelTauTrigger::FillDiTau_(...) - Leading and Sub-Leading MC-Matched Trigger objects have the same eT (" << ldgEt_mcMatched1 << " , " << ldgEt_mcMatched2 << ")! "
	   << "This cannot happen. Exit." << endl;
      exit(1);
    }
  }
...
//****************************************************************************
void PixelTauTrigger::RemoveDuplicates(const vector<L1TkTauParticle> L1TkTaus2, 
...
//****************************************************************************
{
  ...
  if ( L1TkTaus1.size() < L1TkTaus2.size() )
    {
      cout << "E R R O R ! PixelTauTrigger::RemoveDuplicates(...) - Size of L1TkTaus1 (" << L1TkTaus1.size() << ") is smaller than the size of L1TkTaus2 (" << L1TkTaus2.size() << "). "
...
//****************************************************************************
void PixelTauTrigger::GetMcMatchedL1TkTaus(const vector<L1TkTauParticle> L1TkTaus, 
				      vector<L1TkTauParticle> &L1TkTaus_McMatched)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::FillRate_(TH1D *hRate,
...
//****************************************************************************
void PixelTauTrigger::FillRate_(TH2D *hRate,
...
//****************************************************************************
void PixelTauTrigger::FillEfficiency_(TH1D *hEfficiency,
				 const Double_t ldgEt)
//****************************************************************************
{
...
//****************************************************************************
vector<int> PixelTauTrigger::GetTriggerHadronicTaus_(const int tauMom, 					  
...
//****************************************************************************
TLorentzVector PixelTauTrigger::GetHadronicTauVisP4_(const int genP_index)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::GetL1CaloTauMatchGenp(const int calo_index,
				       vector<int> hTaus_index,
				       int &matchGenp_index, 
				       double &matchGenp_deltaR)
//****************************************************************************
...
//****************************************************************************
void PixelTauTrigger::GetL1CaloTauUniqueMatchGenp(vector<L1TkTauParticle> &L1TkTaus)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::FillTurnOn_Numerator_(const vector<L1TkTauParticle> L1TkTaus, 
				       const double minEt,
				       TH1D *hTurnOn)
//****************************************************************************
{
...
//****************************************************************************
void PixelTauTrigger::FillTurnOn_Denominator_(vector<int> mcHadronicTaus,
					 TH1D *hVisEt)
//****************************************************************************
{
...
//**************************************************
void PixelTauTrigger::PrintL1CaloTau(int Indx)
//**************************************************
{
...
//**************************************************
bool PixelTauTrigger::EventWithForbiddenTauDecayMode(string allowedTauDecayMode, 
						vector<int> hTaus_index)
//**************************************************
{
  ...
  for (Size_t i = 0; i < hTaus_index.size(); i++) {
    ...
    if (allowedTauDecayMode.compare("1p") == 0){ if (chargedPions.size() != 1) bFoundForbiddenMode = true;}
      ...
      cout << "E R R O R ! PixelTauTrigger::EventWithForbiddenTauDecayMode(...) - Unknown tau decay mode \"" << allowedTauDecayMode << "\". EXIT" << endl;
      exit(1);
    }
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauMatchTrack(int &matchTk_index,
...
//****************************************************************************
{
  ...
  // Get calo properties
  // Get calo properties
  const double calo_et  = L1CaloTau_Et->at(calo_index);
  const double calo_eta = L1CaloTau_Eta->at(calo_index);
  const double calo_phi = L1CaloTau_Phi->at(calo_index);
  const double calo_e   = L1CaloTau_E->at(calo_index);
  const TLorentzVector calo_p4 = auxTools_.GetTLorentzVector( calo_et, calo_eta, calo_phi, calo_e );
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauMatchPixTrack(int &matchTk_index,
					 const int calo_index,
					 const string matchTk_CollectionType,
					 const double matchTk_minPt,
					 const double matchTk_maxDeltaR,
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauMatchPixTrackRefit(int &matchTk_index,
						   const int calo_index,
						   const string matchTk_CollectionType,
						   const double matchTk_minPt,
						   const double matchTk_maxDeltaR,
...
//****************************************************************************
{
  ...
  // For-Loop: L1Tks
  for (Size_t tk_index = 0; tk_index < L1Tks_Pt->size(); tk_index++) {
    ...
    TVector3 tk_p3      = auxTools_.GetTVector3( L1Tks_Px->at(tk_index), L1Tks_Py->at(tk_index), L1Tks_Pz->at(tk_index) );
    double tk_PtRel     = tk_p3.Perp( calo_p4.Vect() );
    ...
    double pixTkRefit_Pt   = pixFitTk.getMomentum().Perp();
    double pixTkRefit_Eta  = pixFitTk.getMomentum().Eta();
    double pixTkRefit_Phi  = pixFitTk.getMomentum().Phi();
    
    // Apply pT requirement
    bool bIsBelowMinPt   = (pixTkRefit_Pt < matchTk_minPt);
    if (bIsBelowMinPt) continue;

    // Apply track quality criteria
    bool bIsSignalTk = s->SelectPixTracksRefit(pixFitTk, tk_index, matchTk_CollectionType);
    if (!bIsSignalTk) continue;
    
    // Attempt to deltaR-match CaloTau with a track
    const double deltaR = auxTools_.DeltaR(calo_eta, calo_phi, pixTkRefit_Eta, pixTkRefit_Phi);
    
    // Only look at tracks within a matching cone of:  0 <= DeltaR < matchTk_maxDeltaR
    if (deltaR > matchTk_maxDeltaR) continue;

    // Print the TTPixelTrack (refit), TTPixelTrack, and TTTrack properties
    if (0) s->PrintPixTrackRefitProperties(pixFitTk, pixTk_index, true);
    
    // Find track which is closest to the calo. Save deltaR and index
    if (deltaR < matchTk_deltaR) 
      {
	matchTk_pt     = pixTkRefit_Pt;
	matchTk_deltaR = deltaR;
	matchTk_index  = pixTk_index;
	matchTk_PtRel  = tk_PtRel;
      }
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauIsolation(const int matchTk_index,
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauPixIsolation(const int matchTk_index,
					     const vector<int> isoTks_index,
					     double &relIso,
					     double &vtxIso)
//****************************************************************************
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauPixIsolationRefit(const int matchTk_index,
						  const vector<int> isoTks_index,
						  double &relIso,
						  double &vtxIso,
						  L1PixelTrackFit f)
//****************************************************************************
{
  ...
  const double tk_Eta  = L1Tks_Eta->at(matchTk_index);
  const double tk_Phi  = L1Tks_Phi->at(matchTk_index);
  const double tk_RInv = L1Tks_RInv->at(matchTk_index);
  const double tk_t    = sinh(tk_Eta);
  const double tk_d0   = s->GetTrackD0 (matchTk_index);
  const double tk_z0   = L1Tks_POCAz->at(matchTk_index);
  int pixTk_index      = s->GetPixelIndexOfTrack(matchTk_index);
  if (pixTk_index < 0) return;
        
  // Refit the TTTrack to get a new TTPixelTrack
  vector<double> candPixHits_X    = L1PixTks_CandPixHits_X->at(pixTk_index);
  vector<double> candPixHits_Y    = L1PixTks_CandPixHits_Y->at(pixTk_index);
  vector<double> candPixHits_Z    = L1PixTks_CandPixHits_Z->at(pixTk_index);
  vector< int >  candPixHits_Type = L1PixTks_CandPixHits_Type->at(pixTk_index);
  ...
  // Get matching track properties  
  double matchTk_pt    = pixFitTk.getMomentum().Perp();
  double matchTk_POCAz = pixFitTk.getZ0();
  
  // For-loop: IsoTks (TTTrack indices)
  for (Size_t iTk = 0; iTk < isoTks_index.size(); iTk++) { 

    int isoTk_index = isoTks_index.at(iTk);
    
    // Get TTPixelTrack properties
    double tk_Eta   = L1Tks_Eta->at(isoTk_index);
    double tk_Phi   = L1Tks_Phi->at(isoTk_index);
    double tk_RInv  = L1Tks_RInv->at(isoTk_index);
    double tk_t     = sinh(tk_Eta);
    double tk_d0    = s->GetTrackD0(isoTk_index);
    double tk_z0    = L1Tks_POCAz->at(isoTk_index);
    int pixTk_index = s->GetPixelIndexOfTrack(isoTk_index);
    if (pixTk_index < 0) return;
    ...
    // Get the refitted TTPixelTrack properties
    double isoTk_Pt    = L1PixTks_Pt->at(isoTk_index);
    double isoTk_POCAz = L1PixTks_POCAz->at(isoTk_index);
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauAlgoConeSizes(const int calo_index,
...
//****************************************************************************
{
  ...
  if (algoType.compare("FixedCone") == 0){ 
    ...
    cout << "E R R O R ! PixelTauTrigger::GetL1TkTauAlgoConeSizes(...) - Unknown algorithm type \"" << algoType << "\". EXIT" << endl;
    exit(1);
  }
...
#ifdef DEBUG
  cout << "I N F O ! PixelTauTrigger::GetL1TkTauAlgoConeSizes(...) - New max signal cone (min isolation cone ) size is " << sig_maxDeltaR << " (" << iso_minDeltaR << ")" << endl;
#endif
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauSigConeTracks(const int matchTk_index, 
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauSigConePixTracks(const int matchTk_index, 
						 const double deltaR_min,
						 const double deltaR_max,
						 const string selectionType, 
						 vector<int> &sigTks_index)
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauSigConePixTracksRefit(const int matchTk_index, 
						      const double deltaR_min,
						      const double deltaR_max,
						      const string selectionType, 
						      vector<int> &sigTks_index,
...
//****************************************************************************
{
  ...
  // Add the matchTk to the sigTks
  if (matchTk_index == -1) return;
  else sigTks_index.push_back(matchTk_index); // this refers to the TTPixelTrack index prior to refitting
  ...
  // Get TTPixelTrack-Refit properties
  const double matchTk_pt    = pixFitTk.getMomentum().Perp();
  const double matchTk_eta   = pixFitTk.getMomentum().Eta();
  const double matchTk_phi   = pixFitTk.getMomentum().Phi();
  const double matchTk_mass  = constants::pionMass;
  const double matchTk_POCAz = pixFitTk.getZ0();
  TVector3 matchTk_p3        = pixFitTk.getMomentum();
  TLorentzVector tau_p4;
  tau_p4.SetPtEtaPhiM(matchTk_pt, matchTk_eta, matchTk_phi, matchTk_mass);
  ...
  for (Size_t iTk = 0; iTk < L1Tks_Pt->size(); iTk++) {
    ...
    const double tk_Eta  = L1Tks_Eta->at(iTk);
    const double tk_Phi  = L1Tks_Phi->at(iTk);
    const double tk_RInv = L1Tks_RInv->at(iTk);
    const double tk_t    = sinh(tk_Eta);
    const double tk_d0   = s->GetTrackD0 (iTk);
    const double tk_z0   = L1Tks_POCAz->at(iTk);
    int pixTk_index      = s->GetPixelIndexOfTrack(iTk);
    if (pixTk_index < 0) return;
    ...
    bool bIsQualityTk = s->SelectPixTracksRefit(pixFitTk, iTk, selectionType);
    if (!bIsQualityTk) continue;
    ...
    double pixTk_phi   = pixFitTk.getMomentum().Phi();
    double pixTk_POCAz = pixFitTk.getZ0();
    double deltaR      = auxTools_.DeltaR(matchTk_eta, matchTk_phi, pixTk_eta, pixTk_phi);
    double deltaPOCAz  = abs(matchTk_POCAz - pixTk_POCAz);
    TVector3 tk_p3     = pixFitTk.getMomentum();
    TLorentzVector tmp_p4;
    tmp_p4.SetPtEtaPhiM(pixTk_pt, pixTk_eta, pixTk_phi, constants::pionMass);
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauIsoConeTracks(const int matchTk_index, 
					 const double deltaR_min,
					 const double deltaR_max,
					 const string selectionType,
					 vector<int> &isoTks_index)
...
//****************************************************************************
double PixelTauTrigger::GetL1TkTauLdgTkPt(const string tkCollectionType, 
...
//****************************************************************************
{
  ...
  if ( tkCollectionType.compare("TTPixelTracks") == 0 ) ldgTk_Pt = L1PixTks_Pt->at( sigTks.at(0) );
    ...
    cout << "E R R O R ! PixelTauTrigger::GetL1TkTauLdgTkPt(...) - Unknown sample \"" << mcSample << "\". EXIT" << endl;
    exit(1);
  }
...
//****************************************************************************
TLorentzVector PixelTauTrigger::GetL1TkTauSigTksP4(const string tkCollectionType, 
						   const L1TkTauParticle L1TkTau)
//****************************************************************************
{
  ...
  else if ( tkCollectionType.compare("TTPixelTracks-Refit") == 0 ) sigTks_p4 = s->GetP4FromPixTracksRefit(L1TkTau.sigTks_Index_);
  else if ( tkCollectionType.compare("TTTracks") == 0 ) sigTks_p4 = s->GetP4FromTracks(L1TkTau.sigTks_Index_);
  else{
    cout << "E R R O R ! PixelTauTrigger::GetL1TkTauSigTksP4(...) - Unknown Track Collection Type \"" << tkCollectionType << "\". EXIT" << endl;
    exit(1);
  }
...
//****************************************************************************
TLorentzVector PixelTauTrigger::GetL1TkTauIsoTksP4(const string tkCollectionType, 
					      const L1TkTauParticle L1TkTau)
//****************************************************************************
{
  ...
  else if ( tkCollectionType.compare("TTPixelTracks-Refit") == 0 ) isoTks_p4 = s->GetP4FromPixTracksRefit(L1TkTau.isoTks_Index_);
  else if ( tkCollectionType.compare("TTTracks") == 0 ) isoTks_p4 = s->GetP4FromTracks(L1TkTau.isoTks_Index_);
  else{
    cout << "E R R O R ! PixelTauTrigger::GetL1TkTauIsoTksP4(...) - Unknown Track Collection Type \"" << mcSample << "\". EXIT" << endl;
    exit(1);
  }
...
//****************************************************************************
void PixelTauTrigger::PrintTrackProperties(int tk_index)
//****************************************************************************
{

  if ( tk_CollectionType.compare("TTPixelTracks") == 0 ) s->PrintPixTrackProperties(tk_index);
  else if ( tk_CollectionType.compare("TTPixelTracks-Refit") == 0 ) cout << "I N F O ! PixelTauTrigger::PrintTrackProperties(..) - Skipping" << endl;
  else if ( tk_CollectionType.compare("TTTracks") == 0 ) s->PrintTrackProperties(tk_index);
  else
    {
      cout << "E R R O R ! PixelTauTrigger::PrintTrackProperties(...) - Invalid Track Collection Type \"" << tk_CollectionType << "\". EXIT" << endl;
      exit(1);
    }
...
//****************************************************************************
void PixelTauTrigger::PrintL1TkTauProperties(bool bDebug, 
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauIsoConePixTracks(const int matchTk_index, 
					    const double deltaR_min,
					    const double deltaR_max,
					    const string selectionType,
					    vector<int> &isoTks_index)
...
//****************************************************************************
void PixelTauTrigger::GetL1TkTauIsoConePixTracksRefit(const int matchTk_index,
						      const double deltaR_min,
						      const double deltaR_max,
						      const string selectionType,
						      vector<int> &isoTks_index,
...
//****************************************************************************
{
  ...
  vector<double> candPixHits_Z    = L1PixTks_CandPixHits_Z->at(pixTk_index);
  vector< int >  candPixHits_Type = L1PixTks_CandPixHits_Type->at(pixTk_index);
  TTPixelTrack tk                 = f.FitPixelTrack(tk_RInv, tk_Phi, tk_d0, tk_t, tk_z0, candPixHits_X, candPixHits_Y, candPixHits_Z, candPixHits_Type); 

  // Get the refitted TTPixelTrack properties
  double matchTk_eta   = tk.getMomentum().Eta();
  double matchTk_phi   = tk.getMomentum().Phi();
  double matchTk_POCAz = tk.getZ0();
  ...
  for (Size_t iTk = 0; iTk < L1Tks_Pt->size(); iTk++) {
    ...
    double tk_Eta   = L1Tks_Eta->at(iTk);
    double tk_Phi   = L1Tks_Phi->at(iTk);
    double tk_RInv  = L1Tks_RInv->at(iTk);
    double tk_t     = sinh(tk_Eta);
    double tk_d0    = s->GetTrackD0 (iTk);
    double tk_z0    = L1Tks_POCAz->at(iTk);
    int pixTk_index = s->GetPixelIndexOfTrack(iTk);
    if (pixTk_index < 0) return;
    ...
    // Is tk within or isolation annulus
    double tk_eta   = tk.getMomentum().Eta();
    double tk_phi   = tk.getMomentum().Phi();
    double tk_POCAz = tk.getZ0();
    double deltaR      = auxTools_.DeltaR(matchTk_eta, matchTk_phi, tk_eta, tk_phi);
    bool bIsInsideCone = (deltaR <= deltaR_max) && (deltaR >= deltaR_min);
    if (!bIsInsideCone) continue;
