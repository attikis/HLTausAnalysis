...
class PixelTracking : public TreeAnalyserMC{
 ...
 private:
  ...
  void FillTPMatchedPixTrackHistos(int pixTk_Index);

  void FillPixTrackMatchedTPHistos(int pixTk_Index,
				   int tp_Index);
  ...
  void FillPixelHitHistos(int pixTk_Index,
			  bool bPrintPixInfo = false);
  
  void FillCandPixelHitHistos(int pixTk_Index,
			      bool bPrintPixInfo = false);
